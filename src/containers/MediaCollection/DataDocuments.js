...
import { uploadToS3UsingPresignedUrl, openNotification, getImageData } from 'utils/index'
...
class DataDocuments extends React.Component {
  ...
  onDownloadAppFunc = index => {
    const {
      dataQuery: { loading, data, error }
    } = this.props

    if (loading || error || !data || !data[index]) return

    const mediaURL = _.get(data[index], 'data.url')
    const mediaName = _.get(data[index], 'name')

    getImageData(mediaURL)
      .then(data => {
  ...
  render() {
    ...
    const appsPanelParams = {
      ...
      downloadItem: this.onDownloadAppFunc,
