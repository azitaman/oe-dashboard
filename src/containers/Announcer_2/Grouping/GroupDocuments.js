...
import { EnumScreen, cfgDefaultData } from 'utils/constants'
  ...
  onEditAppFunc = (index, values) => {
    ...
    updateGroupingMuation({
      ...
      update: (proxy, { data: { updateGrouping } }) => {
        try {
          console.log("update::", updateGrouping)
  ...
  onAddFieldFunc = values => {
    ...
    createGroupingMuation({
      ...
      .then(() => {
        ...
        const newGroup = grouping[grouping.length - 1]
        // this.onEditAppFunc(grouping.length - 1, { parent: { type: cfgDefaultData }, path: newGroup.type + newGroup.name, data: { cfg: cfgDefaultData } })
        this.onEditAppFunc(grouping.length - 1, { parent: newGroup.type, path: newGroup.type + newGroup.name, data: { cfg: cfgDefaultData } })
