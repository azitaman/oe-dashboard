...
const MyForm = ({
  ...
  const [updateGroupingMuation] = useMutation(mutations.updateGroupingMuation)
  ...
  const [copyToItem, setCopyToItem] = useState()
  ...
  const commonCopyProps = { field: 'copy', form, clientId: _.get(header, 'client._id'), copyToItem: copyToItem, setCopyToItem: setCopyToItem }
  ...
  const onCopy = () => {
    ...
    Promise.all(promises)
      .then((res) => {
        if (res && res[0]?.data?.createGrouping){
          let result = res[0]?.data?.createGrouping;
          console.log("gdgdf:", comp);
          updateGroupingMuation({
            variables: {
              ...
              assignee: comp.assignee,
              // availability: comp.availability,
              data: comp.data,
              emails: comp.emails,
              parent: result.name,
              path: comp.path,
              phoneNumbers: comp.phoneNumbers,
              rank: comp.rank,
    ...
    <>
      <GrayPanel title="Grouping Settings" bgcolor={background}
        ...
        >
          ...
          <ControlButton Icon={faUpload} tooltip="Publish" placement="bottom" onClick={() => showCopyModal(true)} style={{ float: 'right' }} />
